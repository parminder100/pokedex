...
const MoviesDetails = () =>{
    ...
    useEffect(()=>{
        ...
        const getMoviesDetails = async () =>{
            const data = await fetch(`https://api.themoviedb.org/3/movie/${movieId}?api_key=${apiKey}`);
            const response = await data.json();
            setMovieData(response)
            console.log(response);

            const trailerData = await fetch(`https://api.themoviedb.org/3/movie/${movieId}/videos?api_key=${apiKey}`);
